...
public class ParallelMapping {
    ...
    public ParallelMapping(int maxNumberOfThreads, int minNumberOfThreads, int initialNumberOfThreads) {
        //alex
        ...
        this.barriers.put(CONFLICT_ALL, new CyclicBarrier(getNumCurrentThreads()));

        this.executorThread.put(CONFLICT_ALL, 0);
        reconfBarrier = new CyclicBarrier(initialNumberOfThreads + 1);
...
//Alex inicio 
    ...
    public int getNumMinThreads() {
        return this.minNumberOfThreads;
    }

    public LinkedBlockingQueue[] getQueuesActive() {
        LinkedBlockingQueue[] qAtivas = new LinkedBlockingQueue[getNumCurrentThreads()];
    ...
    public void reconfigureBarrier() {
        //this.numberOfthreads = numT;
        this.barriers.remove(CONFLICT_ALL);
        this.barriers.put(CONFLICT_ALL, new CyclicBarrier(getNumCurrentThreads()));

        reconfBarrier = new CyclicBarrier(getNumCurrentThreads() + 1);
    }

    public boolean addMultiGroup(int groupId, int[] groupsId) {
        if (groupId >= getNumCurrentThreads()) {
    ...
    public LinkedBlockingQueue[] getQueues(int groupID) {
        if (groupID == CONFLICT_NONE) {
            ...
            return queues;
        } else if (groupID < getNumCurrentThreads()) {
