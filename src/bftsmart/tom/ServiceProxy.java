...
public class ServiceProxy extends TOMSender {
    ...
    private int invokeTimeout = 500000000;
    ...
    boolean estragaVisao = false;
    int c = 0;
    Storage st = new Storage(1000);
    Storage st1 = new Storage(1000);
    
    public byte[] invokeOrdered(byte[] request) {
        //estragaVisao = true;
        return invoke(request, TOMMessageType.ORDERED_REQUEST,0);
    }

    public byte[] invokeUnordered(byte[] request) {
        return invoke(request, TOMMessageType.UNORDERED_REQUEST,0);
    }
    ...
    public byte[] invoke(byte[] request, TOMMessageType reqType, int groupId) {
        ...
        if(estragaVisao){
           getViewManager().getCurrentView().setId(getViewManager().getCurrentViewId()-1);
           estragaVisao = false;
        }
        ...
        try {
            if (!this.sm.tryAcquire(invokeTimeout, TimeUnit.MICROSECONDS)) {
                ...
                if(getViewManager().getViewStore().readView().getId() == getViewManager().getCurrentViewId()) {
                    return null;
                }else{
                    
                    //System.out.println("Vai reexecutar...");
                    
                    reconfigureTo(getViewManager().getViewStore().readView());
                     if(c >1000){
                        st1.store(System.nanoTime() - last_send_instant);
                     }
           
                     last_send_instant = System.nanoTime();
                     byte[] r = null;
                     //Tem que ser um invoke com o mesmo ID;
                     //
                     //byte[] r = invoke(request, reqType,groupId);
                     
                     if(c > 1000){
                        st.store(System.nanoTime() - last_send_instant);
                     }

                    if (st.getCount() == 1000 && getProcessId() == 7001){
                        
                        System.out.println(" 2 - atualizar view // Average time for 1000 executions (-10%) = " + st1.getAverage(true) / 1000 + " us ");
                         System.out.println(" 2 // Standard desviation for 1000 executions (-10%) = " + st1.getDP(true) / 1000 + " us ");
                         System.out.println(" 2 // Average time for 1000 executions (all samples) = " + st1.getAverage(false) / 1000 + " us ");
                         System.out.println(" 2 // Standard desviation for 1000 executions (all samples) = " + st1.getDP(false) / 1000 + " us ");
                        
                        
                         System.out.println(" 3 - reenvio // Average time for 1000 executions (-10%) = " + st.getAverage(true) / 1000 + " us ");
                         System.out.println(" 3 // Standard desviation for 1000 executions (-10%) = " + st.getDP(true) / 1000 + " us ");
                         System.out.println(" 3 // Average time for 1000 executions (all samples) = " + st.getAverage(false) / 1000 + " us ");
                         System.out.println(" 3 // Standard desviation for 1000 executions (all samples) = " + st.getDP(false) / 1000 + " us ");
        ...
        if (response == null) {
            ...
            if (reqType == TOMMessageType.ORDERED_REQUEST) {
                ...
                if (response.getViewID() == getViewManager().getCurrentViewId()) {
                    ...
                    //System.out.println("Reconfigurou!!!");
                    ...
                    canSendLock.unlock();
                     ...
                     /*if (getProcessId() == 7001){
                            System.out.println("Reconfigurou!!! "+st.getCount());      
                     }*/
                    
                     if (st.getCount() == 1000 && getProcessId() == 7001){
                         System.out.println(getProcessId() + " 2 // Average time for 1000 executions (-10%) = " + st.getAverage(true) / 1000 + " us ");
                         System.out.println(" 2 // Standard desviation for 1000 executions (-10%) = " + st.getDP(true) / 1000 + " us ");
                         System.out.println(" 2 // Average time for 1000 executions (all samples) = " + st.getAverage(false) / 1000 + " us ");
                         System.out.println(" 2 // Standard desviation for 1000 executions (all samples) = " + st.getDP(false) / 1000 + " us ");
